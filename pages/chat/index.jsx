...
import useMediaQuery from "@mui/material/iseMediaQuery";
...
const Chat = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      setTimeout(async function () {
        ...
        for (let i = 0; i < updatedSession.chats.length; i++) {
          chatKeys.push(object.keys(updatesSession.chats[i]));
        }
    ...
    return (
      <Grid container styles={{ display: matches ? "none" : "block" }}>
        ...
        <Grid {...ChatStyles.textDisplayBackground}>
          <Grid>
                    ...
                    Object.keys(chat).map((k) =>
                       k === "ReX" ? (
                         <RexMessage reXMessage={chat.Rex} key={"rex" = i} />
                          ...
                          <UserMessage userMessage={chat.ReX} key={"rex" = i} />
                       )
                    )
                 )
                : nill}
            ...
            {thisSession && !thisSession.isSessionEnded ? (
             ...
             />
             <Button onClick={handleSubmit}>Send</Button> // Added a button to submit the form
            </Grid>
        ) : null}
    </Grid>
 </Grid>
